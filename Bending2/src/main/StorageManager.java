...
public class StorageManager {
	...
	public String getLanguage(Player player) {
		...
		if (StorageManager.useFlatFile) {
			...
			String getLanguage = "SELECT language  FROM bending_language WHERE player ='"
					+ player.getName() + "'";
	...
	public void setLanguage(Player player, String language) {
		...
		if (StorageManager.useFlatFile) {
			...
			String checkEntry = "SELECT language FROM bending_language WHERE player ='"
					+ player.getName() + "'";
	...
	public void setAbility(Player player, Abilities ability, int slot) {
		...
		if (StorageManager.useFlatFile) {
			...
			String checkAbilities = "SELECT ability FROM bending_ability WHERE setter ='"
	...
	public Abilities getAbility(Player player, int slot) {
		...
		if (StorageManager.useFlatFile) {
			...
			String selectAbility = "SELECT ability FROM bending_ability WHERE setter ='"
					+ setter + "' AND player = '" + player.getName() + "'";
	...
	public void initialize(File file) {
		...
		if (StorageManager.useMySQL) {
			...
			if (this.MySql.initialize()) {
				...
				String createTable3 = "CREATE TABLE IF NOT EXISTS bending_language(player TEXT NOT NULL, language TEXT NOT NULL)";
